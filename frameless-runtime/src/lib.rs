...
pub enum Calls {
	SetCode(Vec<u8>), // Keep it as first variant!!
	Add(u32),
	Mul(u32),
	KillSwitch,
	Hello(u8),
...
// 666C6970706572 flipper raw storage key
pub const FLIPPER_KEY: [u8; 7] = *b"flipper";
// 616363 acc raw storage key
pub const ACC_KEY: [u8; 3] = *b"acc";
...
impl_runtime_apis! {
	...
	impl sp_block_builder::BlockBuilder<Block> for Runtime {
		fn apply_extrinsic(extrinsic: <Block as BlockT>::Extrinsic) -> ApplyExtrinsicResult {
			...
			// FLipper
			let raw_flip = sp_io::storage::get(&FLIPPER_KEY).unwrap_or(vec![0x00]); // should use exists instead of unwrap ?
			let flip = bool::decode(&mut &*raw_flip).expect("can't decode flipper storage value");
			sp_io::storage::set(&FLIPPER_KEY, &(!flip).encode());
			info!(target: "frameless", "🖼️ FLIPPING: {}", !flip);

			let call = extrinsic.0;

			match call {
				Calls::SetCode(new_code) => {
					// bump spec_version here possible? don't think so
					// Please do framlessly first Self::can_set_code(&code)?;

					sp_io::storage::set(&sp_storage::well_known_keys::CODE, &new_code);

					// Below doesn't work need frame_support
					//sp_io::storage::unhashed::put_raw(&sp_storage::well_known_keys::CODE, &new_code);
				}
				Calls::Add(nb) => {
					let raw_acc = sp_io::storage::get(&ACC_KEY).unwrap_or(vec![0u8; 4]); // should use exists instead of unwrap ?
					let mut acc = u32::decode(&mut &*raw_acc).expect("can't decode acc storage value");
					acc = nb + 10;
					sp_io::storage::set(&ACC_KEY, &acc.encode());
				},
				Calls::Mul(nb) => {
					let raw_acc = sp_io::storage::get(&ACC_KEY).unwrap_or(vec![0u8; 4]); // should use exists instead of unwrap ?
					let mut acc = u32::decode(&mut &*raw_acc).expect("can't decode acc storage value");
				...
				Calls::KillSwitch => {
					sp_io::storage::clear(&sp_storage::well_known_keys::CODE);
				}
				Calls::Hello(nb) => {
					info!(target: "frameless", "🖼️ IN HELLO");
				}
				/*
				Calls::GoodBye(nb) => {
					info!(target: "frameless", "🖼️ IN GOODBYE");
				}*/
